...
// 这同以前建立事件监听对象的方法一样的效果，
        searchBnt.setOnClickListener(onClickListener);
...
//使用 ContentResolver 的 query()方法执行查询，结果保存在 cursor 对象中
...
//使用 ContentResolver 查找联系人的 E-mail 地址
                Cursor emails =
                        getContentResolver().query(ContactsContract.CommonDataKinds.Email.CONTENT_URI,
                                null,
                                ContactsContract.CommonDataKinds.Email.CONTACT_ID + "=" + contractId,
                                null,
                                null);
//遍历该联系人的多个 E-mail 地址
...
//向 RawContacts.CONTENT_URI 执行一个空值插入，目的是获取系统返回的 rawContactId
...
//向联系人 URI 添加联系人名字
            getContentResolver().insert(ContactsContract.Data.CONTENT_URI, values);//加入 phone
            values.clear();
