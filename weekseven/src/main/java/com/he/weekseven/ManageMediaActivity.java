...
//用 ContentResolver 对象的 insert()方法插入数据，返回所插入数据对应的 uri
            Uri uri = getContentResolver().insert(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values);
//加载应用程序 drawable 目录中的 belltower.jpeg 图片
            Bitmap bitmap = BitmapFactory.decodeResource(
                    ManageMediaActivity.this.getResources(),
                    R.drawable.belltower);
//定义输出流
...
//第二步：创建一个 SimpleAdapter，将其设置为要显示结果的 ListView 的数据适配器
...
//使用对话框显示用户单击的图片
                    new AlertDialog.Builder(ManageMediaActivity.this).
                            setView(viewDialog).
